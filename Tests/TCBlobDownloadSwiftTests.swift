...
class Httpbin {
    class func status(_ status: Int) -> URL {
        return URL(string: "status/\(status)", relativeTo: kHttpbinURL)!
    }
    class func fixtureWithBytes(_ bytes: Int = 20) -> URL {
...
class TCBlobDownloadManagerTests: XCTestCase {
    ...
    func testDownloadFileAtURLWithDelegate_to_directory() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(), toDirectory: kTestsDirectory, withName: "first_test", andDelegate: downloadHandler)
    ...
    func testDownloadFileAtURLWithClosures_to_directory() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(), toDirectory: kTestsDirectory, withName: "first_test", progression: nil) { (download, error, location) -> Void in
            XCTAssertNil(error, "Download failed with error.")
            XCTAssertNotNil(location, "Successful download didn't send the location parameter")
            expectation.fulfill()
        }
    ...
    func testDownloadFileAtURLWithDelegate_call_methods() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(), toDirectory: kTestsDirectory, withName: nil, andDelegate: downloadHandler)
    ...
    func testDownloadFileAtURLWithClosures_call_methods() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(), toDirectory: kTestsDirectory, withName: nil, progression: { (progress, totalBytesWritten, totalBytesExpectedToWrite) -> Void in
            didCallProgress = true
        }) { (download, error, location) -> Void in
            expectation.fulfill()
        }
    ...
    func testDownloadFileAtURLWithDelegate_methods_parameters() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(10), toDirectory: kTestsDirectory, withName: nil, andDelegate: downloadHandler)
    ...
    func testDownloadFileAtURLWithClosures_parameters() {
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.fixtureWithBytes(10), toDirectory: kTestsDirectory, withName: nil, progression: { (progress, totalBytesWritten, totalBytesExpectedToWrite) -> Void in
            XCTAssert(10 == totalBytesWritten)
            XCTAssert(10 == totalBytesExpectedToWrite)
            XCTAssert(1.0 == progress)
        }) { (download, error, location) -> Void in
    ...
    func testDownloadFileAtURLWithDelegate_invalid_response() {
        ...
        class DownloadHandler: NSObject, TCBlobDownloadDelegate {
            ...
            func download(_ download: TCBlobDownload, didFinishWithError error: NSError?, atLocation location: URL?) {
                ...
                XCTAssert(error?.userInfo[kTCBlobDownloadErrorHTTPStatusKey] as? NSValue == NSNumber(value: 404), "Error userInfo is missing status")
        ...
        let _ = TCBlobDownloadManager.sharedInstance.downloadFileAtURL(Httpbin.status(404), toDirectory: kTestsDirectory, withName: nil, andDelegate: downloadHandler)
